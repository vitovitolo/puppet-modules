define dns::environment_reverse_master_zone
(
	...
	$reverse_range = hiera("dns::zone::${name}::reverse_range", ""),
...
)
{
	...
	dns::master_zone_data{"zone_${name}_environment_reverse_data":
		zone    => "$name",
		content => template("ds/db.zone.environment.reverse.erb"),
		order   => "200",
	}
}
